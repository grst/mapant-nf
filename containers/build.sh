...
#
#   process {
#       withName: 'PLAN_GRIDS|RENDER_INI|MAKE_VECTOR_TILES|MERGE_PMTILES|MAKE_VIEWER' { container = 'mapant/tiler:latest' }
...
# The karttapullautin commit to build, read out of the Containerfile's own ARG default rather than
# repeated here: it is part of the image's build inputs, and CI decides whether an image needs
# rebuilding from the hash of exactly those inputs. A copy in this script would be a way for the two
# to disagree -- bump the ARG and CI would rebuild with the old commit.
readonly PULLAUTA_REF="${PULLAUTA_REF:-$(
    sed -n 's/^ARG PULLAUTA_REF=//p' "${SCRIPT_DIR}/karttapullautin/Containerfile"
)}"
[ -n "$PULLAUTA_REF" ] || {
    echo 'FATAL: no ARG PULLAUTA_REF in containers/karttapullautin/Containerfile' >&2
    exit 1
}

# Image name -> tag. The karttapullautin tag names the commit it is built from.
declare -rA TAGS=(
    [karttapullautin]="${PULLAUTA_REF:0:7}"
    [gdal]='latest'
    [osmium]='latest'
    [tiler]='latest'
)
...
declare -rA BUILD_ARGS=(
    [karttapullautin]="PULLAUTA_REF=${PULLAUTA_REF}"
)
...
for name in "${images[@]}"; do
    ...
    # karttapullautin compiles three ISA variants from a source checkout, so the commit it is
    ...
    # images can name, so switching to them needs no knowledge of which commit they came from.
    tags=(-t "$ref")
